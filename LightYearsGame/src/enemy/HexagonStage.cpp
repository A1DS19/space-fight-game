...
HexagonStage::HexagonStage(World *world)
    : GameStage{world}, mSpawnInterval{5.f}, mSideSpawnOffset{80.f},
      mSpawnGroupAmt{5}, mCurrentSpawnCount{0},
      mMidSpawnLoc{world->GetWindowSize().x / 2.f, -100.f},
      mRightSpawnLoc{world->GetWindowSize().x / 2.f + mSideSpawnOffset,
                     -100.f - mSideSpawnOffset},
      mLeftSpawnLoc{world->GetWindowSize().x / 2.f - mSideSpawnOffset,
                    -100.f - mSideSpawnOffset} {}

void HexagonStage::StartStage() {
  mSpawnTimer = TimerManager::Get().SetTimer(
      GetWeakRef(), &HexagonStage::SpawnHexagon, mSpawnInterval, true);
}
