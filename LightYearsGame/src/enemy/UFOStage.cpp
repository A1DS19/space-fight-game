...
UFOStage::UFOStage(World *world)
    : GameStage{world}, mSpawnInterval{4.f}, mSpawnAmt{10},
      mCurrentSpawnAmt{0}, mUFOSpeed{200.f} {}

void UFOStage::StartStage() {
  mSpawnTimer = TimerManager::Get().SetTimer(GetWeakRef(), &UFOStage::SpawnUFO,
                                             mSpawnInterval, true);
}

void UFOStage::StageFinished() { TimerManager::Get().ClearTimer(mSpawnTimer); }
